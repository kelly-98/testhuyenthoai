(function (n) {
  ...
  b.initRoot = function () {
    ...
    b.root.setAttribute(
      "viewBox",
      a.xmin + " " + a.ymin + " " + n.FrameWidth + " " + n.FrameHeight
    );
  ...
  b.processText = function (a) {
    for (
      ...
      if (a.texttype == "html") {
        ...
        f.fontid.toString().match(/\d/)
          ? h.setAttribute("font-family", "ft" + f.fontid)
          : h.setAttribute("font-family", f.fontid);
  ...
  b.processShape = (function () {
    var a = function (a, c, d) {
        ...
        d = b.createTag(d);
        for (var f in a)
          typeof a[f] == "object" || f == "type" || d.setAttribute(f, a[f]);
        a = a.stop;
        f = 0;
      ...
      for (var i = 0, o = g.length; i < o; i++) {
        ...
        if (typeof h.fill !== "undefined") {
          ...
          if (j) {
            ...
            l = j.type;
            if (l == 1)
              q.setAttribute("fill", j.color),
                q.setAttribute(
                  "fill-opacity",
                  typeof j["fill-opacity"] == "undefined"
                    ? 1
                    : j["fill-opacity"]
                );
            else {
  ...
  b.filterElement = function (a, d) {
    ...
    this.Shadow = function (a) {
      this.Rgb(c(this.base))
        .Fg(a.blurx, a.blury)
        .Fo(a.dx, a.dy)
        .Rgba(a.strength, a.color, "shadow")
        .Com("over", c(this.base), "shadow", ++this.base);
    };
    ...
    this.Glow = function (a) {
      this.Rgb(c(this.base))
        .Fg(a.blurx, a.blury)
        .Rgba(a.strength, a.color, ++this.base)
        .Com("over", c(this.base - 1), this.base, ++this.base);
    };
    this.Bevel = function (a) {
      this.Rgb(c(this.base))
        .Fg(a.blurx, a.blury)
        .Fo("-" + a.dx, "-" + a.dy)
        .Rgba(a.strength, a.highlightColor, ++this.base)
        .Rgb(c(this.base - 1))
        .Fg(a.blurx, a.blury)
        .Fo(a.dx, a.dy)
        .Rgba(a.strength, a.color, ++this.base)
        .Com("xor", this.base, c(this.base - 1), "shadow")
        .Com("over", "shadow", c(this.base - 2), ++this.base);
    };
    this.init = function () {
  ...
  b.sprite = function (a, d) {
    ...
    this.par = d;
    this.frames = [];
    for (var e = [], c = 0, g = this.placeObjects.length; c < g; c++) {
      var f = this.placeObjects[c];
      f.type == 14 ||
        f.type == 43 ||
        (e.push(f), f.type == 1 && (this.frames.push(e), (e = [])));
    }
  ...
  b.sprite.prototype.showFrame = function () {
    for (
      var a = this.frames[this.frame], d = null, b = 0, c = a.length - 1;
      b < c;
      b++
    )
      (d = a[b]), this.renderShape(d), (this.lastDepth = d.depth);
    this.lastDepth = null;
  ...
  b.sprite.prototype.action = function (a) {
    a = a.aslist;
    for (var d = 0, b = 0, c = a.length; b < c; b++)
      switch (((d = a[b].code), d)) {
        case 7:
          this.stop();
          break;
  ...
  b.sprite.prototype.renderShape = function (a) {
    ...
    this.afterRender(
      "replace" in a ? this.replaceShape(a) : this.getShape(a),
      a
    );
    return !0;
  };
  b.sprite.prototype.replaceShape = function (a) {
    ...
    if (this.depths[c]) {
      ...
      if (g == b.id) return f;
      d.replaceChild(b, f);
      this.sprites[c + ":" + this.depths[c]] &&
        ((this.sprites[c + ":" + this.depths[c]] = null),
        delete this.sprites[c + ":" + this.depths[c]]);
      this.buttons &&
        this.buttons[c + ":" + this.depths[c]] &&
        ((this.buttons[c + ":" + this.depths[c]] = null),
        delete this.buttons[c + ":" + this.depths[c]]);
    } else
      this.lastDepth
        ? ((g =
          ...
          (g = document.getElementById(g).nextSibling))
        : (g = d.firstChild),
        g ? d.insertBefore(b, g) : d.appendChild(b);
    this.depths[c] = a.id;
    return b;
  };
  b.sprite.prototype.removeShape = function (a) {
    if (this.depths[a]) {
      ...
      d = this.par.getAttribute("id") + ":" + a + ":" + d;
  ...
  b.sprite.prototype.moveShape = function (a, d) {
    ...
    if (a.nodeName.toLowerCase() == "g")
      ...
      for (var c = a.childNodes, g = 0, f = c.length; g < f; g++)
        c[g].setAttribute("transform", "matrix(" + b + ")");
  };
  b.sprite.prototype.moveText = function (a, d) {
    for (
      var e = a.firstChild.nextSibling.childNodes, c = d.matrix, g = 0;
      e[g];
      g++
    ) {
  ...
  b.sprite.prototype.moveButton = function (a, d) {
    ...
    this.buttons[e + ":" + c] ||
      (this.buttons[e + ":" + c] = new b.sprite.Button(a, f));
    this.buttons[e + ":" + c].element.setAttribute(
      "transform",
      "matrix(" + g + ")"
    );
  };
  ...
  b.sprite.Button.prototype = {
    init: function (a) {
      for (var d = a.record, e, c, g, f = d.length - 1; f >= 0; f--)
        (e = d[f]),
          (g = e.state),
          (c = b.defineShapes[e.shapeid].cloneNode(!0)),
          c.setAttribute("transform", e.transform),
          c.setAttribute("sid", e.shapeid),
          c.setAttribute("id", this.pid.concat(":", f, ":", e.shapeid)),
          this.states[g] || (this.states[g] = []),
          this.states[g].push(c);
      if ("up" in this.states) {
        c = null;
        f = 0;
        for (d = this.states.up.length; f < d; f++)
          if (
            ((c = this.states.up[f].cloneNode(!0)),
            (c.style.display = "block"),
            c.setAttribute("state", "up"),
            this.element.appendChild(c),
            c.getAttribute("type") == "sprite")
          )
            (e = c.getAttribute("sid")),
              (this.sprites.up = this.sprites.up || new b.sprite(b.hash[e], c)),
              this.sprites.up.showFrame();
      }
      if ("hit" in this.states) {
        ...
        for (d = this.states.hit.length; f < d; f++)
          if (
            ((c = this.states.hit[f].cloneNode(!0)),
            c.setAttribute("opacity", 0),
            c.setAttribute("state", "hit"),
            this.element.appendChild(c),
            c.getAttribute("type") == "sprite")
          )
            (e = c.getAttribute("sid")),
              (this.sprites.hit =
                this.sprites.hit || new b.sprite(b.hash[e], c));
      }
      if ("down" in this.states) {
        ...
        for (d = this.states.down.length; f < d; f++)
          if (
            ((c = this.states.down[f].cloneNode(!0)),
            ...
            (e = c.getAttribute("sid")),
              (this.sprites.down =
                this.sprites.down || new b.sprite(b.hash[e], c)),
              this.sprites.down.showFrame();
      }
      if ("over" in this.states) {
        c = null;
        f = 0;
        for (d = this.states.over.length; f < d; f++)
          if (
            ((c = this.states.over[f].cloneNode(!0)),
            (c.style.display = "none"),
            c.setAttribute("state", "over"),
            this.element.appendChild(c),
            c.getAttribute("type") == "sprite")
          )
            (e = c.getAttribute("sid")),
              (this.sprites.over =
                this.sprites.over || new b.sprite(b.hash[e], c)),
              this.sprites.over.showFrame();
      }
      this.normalEvents();
    ...
    changeState: function (a) {
      if (a in this.states)
        for (var d = this.element.childNodes, b = 0, c; (c = d[b]); b++)
          c.style.display =
            c.getAttribute("state") == "hit" || c.getAttribute("state") == a
              ? "block"
              : "none";
    },
  };
  ...
  b.sprite.prototype.colorTransform = (function () {
    function a(a, b) {
      var c = a.match(/rgb\(([^(]+)\)/)[1].split(","),
        g = Math.max(
          0,
          Math.min(((c[0] - 0) * b.RedMultTerm) / 256 + b.RedAddTerm, 255)
        ),
        f = Math.max(
          0,
          Math.min(((c[1] - 0) * b.GreenMultTerm) / 256 + b.GreenAddTerm, 255)
        );
      c = Math.max(
        0,
        Math.min(((c[2] - 0) * b.BlueMultTerm) / 256 + b.BlueAddTerm, 255)
      );
      g = parseInt(g);
      f = parseInt(f);
      c = parseInt(c);
    ...
    return function (b, e) {
      ...
      if (
        (c == "shape" || c == "text") &&
        b.nodeName.toLowerCase() != "clippath"
      ) {
  ...
  b.sprite.prototype.addFilters = function (a, d) {
    if ("filters" in d) {
      var e = d.filters;
  ...
  b.sprite.prototype.createShape = function (a) {
    ...
    "clipDepth" in a
      ? ((this.clipDepthId = e),
        (this.clipDepth = a.clipDepth),
        (this.clipDepthDepth = a.depth),
        (b = this.createClip(a)))
      : this.clipDepthId &&
        a.depth > this.clipDepthDepth &&
        a.depth <= this.clipDepth &&
        b.setAttribute("clip-path", "url(#" + this.clipDepthId + ")");
    b.setAttribute("id", e);
    return b;
  };
